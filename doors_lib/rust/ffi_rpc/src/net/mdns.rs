...
use crate::net::partner_service_info::PartnerServiceInfo;
...
impl Mdns {
    ...
    pub fn register_service(&mut self)-> Result<(),anyhow::Error> {
        ...
        let responder = libmdns::Responder::new_with_ip_list(addrs)?;
        let _service = responder.register_with_ttl(&self.service_info.service_type, &self.service_info.instance_name, self.service_info.port,&[""],1024);
        Ok(())
    }
